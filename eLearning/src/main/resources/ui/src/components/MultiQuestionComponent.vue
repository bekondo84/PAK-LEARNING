    ...
    <v-card-item>
      <div class="truefalse">
        ...
        <div>
            ...
            <v-radio v-for="answer in question.question" :key="answer.pk"  :value="answer.pk">
                <template v-slot:label>
                  <div>{{ answerItem(answer) }}</div>
                  <mark-question v-if="showCorrectAnswer" :state="evaluateAnwser(answer)"></mark-question>
                </template>
            </v-radio>
            </v-radio-group>
        </div>
      ...
      MarkQuestion: defineAsyncComponent(() => import('@/components/MarkQuestionComponent.vue')),
        ...
        translater(key: string) {
          ...
          if (this.radios == null) {
             ...
             this.overlay = true ;
             try {
               const response = await axiosService.evaluateMulti(""+this.radios, this.item.pk);
       ...
       if (answer != null) {
          this.radios = answer.pk ;
       }
       this.translate = await axiosService.translate(['answer.selection.message', 'validate.btn.name', 'connector.to.name', 'Resource.name', 'Resource.description', 'exit.btn.label', 'Activity.state']);
       this.overlay = false ;
    }
})
